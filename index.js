  ...
  static unpack_common() {
    ...
    exportMethods.forEach(function(expmthd) {
      if (expmthd.name.indexOf("OpenCommon") > -1 || expmthd.name.indexOf("OpenMemory") > -1) {
        ...
        Interceptor.attach(expmthd.address, {
          onEnter: function(args) {
            if (args[1].readU32() == _fridaUnpack.DEX_MAGIC) {
              _fridaUnpack.dexrec.push(args[1]);
            }
          }
        });
      }
    ...
    if (frida_java_bridge_default.available) {
      frida_java_bridge_default.perform(function() {
        ...
        var application = frida_java_bridge_default.use("android.app.Application");
        var BaseDexClassLoader = frida_java_bridge_default.use("dalvik.system.BaseDexClassLoader");
        ...
        var reflectField = frida_java_bridge_default.use("java.lang.reflect.Field");
        var reflectMethod = frida_java_bridge_default.use("java.lang.reflect.Method");
        var reflectObject = frida_java_bridge_default.use("java.lang.Object");
        var reflectClass = frida_java_bridge_default.use("java.lang.Class");
        var reflectString = frida_java_bridge_default.use("java.lang.String");
        var reflectClassloader = frida_java_bridge_default.use("java.lang.ClassLoader");
        if (application != void 0) {
          application.attach.overload("android.content.Context").implementation = function(context) {
            ...
            if (classloader != void 0) {
              var casedloader = frida_java_bridge_default.cast(classloader, BaseDexClassLoader);
              ...
              var castedEnumerateClass = frida_java_bridge_default.cast(EnumerateClass, reflectClass);
              ...
              if (loadAllClass != void 0) {
                ...
                var args = frida_java_bridge_default.array("Ljava.lang.Object;", [classloader]);
                var classlist = loadAllClass.invoke(null, args);
                console.log("start dump dex ");
                for (var i in _fridaUnpack.dexrec) {
                  if (_fridaUnpack.dexrec[i].readU32() == _fridaUnpack.DEX_MAGIC) {
                    var dex_len = _fridaUnpack.dexrec[i].add(32).readU32();
                    var dumppath = filesDir.toString() + "/" + dex_len.toString(16) + ".dex";
                    console.log(dumppath);
                    var dumpdexfile = new File(dumppath, "wb");
                    dumpdexfile.write(_fridaUnpack.dexrec[i].readByteArray(dex_len));
                    dumpdexfile.close();
...
(function(Anti2) {
  ...
  function anti_exit() {
    ...
    Interceptor.replace(exit_ptr, new NativeCallback(function(code3) {
      if (null == this) {
        return 0;
      }
  ...
  function anti_kill() {
    ...
    Interceptor.replace(kill_ptr, new NativeCallback(function(ptid, code3) {
      if (null == this) {
        return 0;
      }
      var lr = FCCommon.getLR(this.context);
...
(function(FCAnd2) {
  ...
  function hook_Map(pKey, accurately) {
    ...
    const LinkedHashMap = frida_java_bridge_default.use("java.util.LinkedHashMap");
  ...
  function dump_dex_loadAllClass() {
    ...
    function loadAllClass2() {
      ...
      function loadAllClassCore(loader) {
        ...
        var class_BaseDexClassLoader = frida_java_bridge_default.use("dalvik.system.BaseDexClassLoader");
        var pathcl = frida_java_bridge_default.cast(loader, class_BaseDexClassLoader);
        DMLog.i(tag3, ".pathList: " + pathcl.pathList.value);
        var class_DexPathList = frida_java_bridge_default.use("dalvik.system.DexPathList");
        var dexPathList = frida_java_bridge_default.cast(pathcl.pathList.value, class_DexPathList);
        DMLog.i(tag3, ".dexElements: " + dexPathList.dexElements.value.length);
        var class_DexFile = frida_java_bridge_default.use("dalvik.system.DexFile");
        var class_DexPathList_Element = frida_java_bridge_default.use("dalvik.system.DexPathList$Element");
        for (var i = 0; i < dexPathList.dexElements.value.length; i++) {
          var dexPathList_Element = frida_java_bridge_default.cast(dexPathList.dexElements.value[i], class_DexPathList_Element);
          if (dexPathList_Element.dexFile.value) {
            var dexFile = frida_java_bridge_default.cast(dexPathList_Element.dexFile.value, class_DexFile);
  ...
  const tjm_default_white_detail = {
  ...
  function traceJavaMethods(clazzes, clsWhitelist, stackFilter) {
    let dest_cls = [];
    let dest_white = { ...tjm_default_white_detail, ...clsWhitelist };
    if (clazzes != null) {
  ...
  function traceJavaMethods_custom(clazzes, clsWhitelist, stackFilter) {
    ...
    function traceJavaMethodsCore(clsname) {
      ...
      let cls = frida_java_bridge_default.use(clsname);
          ...
          methodOverloads.forEach(function(overload) {
            try {
              ...
              overload.implementation = function() {
                let tid = Process.getCurrentThreadId();
                let tname = frida_java_bridge_default.use("java.lang.Thread").currentThread().getName();
                sendContent({
                  tid,
                  status: "entry",
                  tname,
    ...
    frida_java_bridge_default.enumerateLoadedClassesSync().forEach((curClsName, index, array) => {
      ...
      for (let i = 0; i < declaredFields.length; i++) {
        ...
        if (null != fdata) {
          if (type.getName() != "[B") {
            ...
            fdata = frida_java_bridge_default.array("byte", fdata);
  ...
  function useWhenLoadClass(clsname, callback) {
    ...
    ClassLoader.loadClass.overload("java.lang.String").implementation = function(name) {
      ...
      if (name.indexOf(clsname) > -1) {
        ...
        try {
          const clsFactory = frida_java_bridge_default.ClassFactory.get(this);
  ...
  function useWithInMemoryDexClassLoader(clsname, callback) {
    ...
    try {
      ...
      InMemoryDexClassLoader.$init.overload("java.nio.ByteBuffer", "java.lang.ClassLoader").implementation = function(buff, loader) {
        ...
        let clsFactory = frida_java_bridge_default.ClassFactory.get(this);
        try {
      ...
      onMatch(loader) {
        try {
          ...
          if (null != cls) {
            ...
            let cf = frida_java_bridge_default.ClassFactory.get(loader);
            callback(cf);
          }
        } catch (e) {
  ...
  function getProcessName() {
    ...
    var read2 = new NativeFunction(readPtr, "int", ["int", "pointer", "int"]);
    ...
    if (fd != -1) {
      ...
      var readsize = read2(fd, buffer, 4096);
      close(fd);
  ...
  exports.tjm_default_cls = tjm_default_cls;
  exports.tjm_default_white_detail = tjm_default_white_detail;
  exports.anti = anti;
  exports.jni = jni;
...
function isProbablyUtf8(buffer) {
  const BufferCls = frida_java_bridge_default.use("okio.Buffer");
  const Character = frida_java_bridge_default.use("java.lang.Character");
...
function hookInterceptor(name) {
  frida_java_bridge_default.perform(function() {
    const CallServerInterceptor = frida_java_bridge_default.use(name);
    const BufferCls = frida_java_bridge_default.use("okio.Buffer");
    const GzipSource = frida_java_bridge_default.use("okio.GzipSource");
    CallServerInterceptor.intercept.implementation = function(chain) {
      ...
      const startNs = frida_java_bridge_default.use("java.lang.System").nanoTime();
      let response;
      try {
      ...
      const tookMs = (frida_java_bridge_default.use("java.lang.System").nanoTime() - startNs) / 1e6;
      ...
      source.request(frida_java_bridge_default.use("java.lang.Long").MAX_VALUE.value);
      let buffer = source.buffer();
      buffer = frida_java_bridge_default.cast(buffer, BufferCls);
      let gzippedLength = null;
      if (encoding !== null && encoding.toLowerCase() === "gzip") {
        gzippedLength = buffer.size();
        const gzipSource = GzipSource.$new(frida_java_bridge_default.cast(buffer.clone(), BufferCls));
      ...
      if (contentLength !== 0) {
        const bodyText = frida_java_bridge_default.cast(buffer.clone(), BufferCls).readUtf8();
...
function hookMethods(targetClass, targetMethod, start = null, end = null) {
  try {
    ...
    var hook = frida_java_bridge_default.use(targetClass);
    console.log(hook);
...
function hookClassMethods(className, methodName) {
  const targetClass = frida_java_bridge_default.use(className);
